...
contract Dex is Wallet{
    ...
    function placeMarketOrder(bytes32 _symbol, uint256 _amount, Side _side) public tokenexists(_symbol) {
        ...
        while( _amount > 0 ){
            ...
            if(_side == Side.Sell){
                //find out if require means that even if we already used other functions, and adjusted balances, if eventually there are no buy orders if everything gets reverted.
                require(Orderbook[_symbol][uint256(Side.Buy)].length >0, "There are no more buy orders, so you cant sell at market");
                price = Orderbook[_symbol][uint256(Side.Buy)][0].price;
                amountToCompare = Orderbook[_symbol][uint256(Side.Buy)][0].amount;
            }
            else{
                require(Orderbook[_symbol][uint256(Side.Sell)].length >0, "There are no more sell orders, so you cant buy at market");
